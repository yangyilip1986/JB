...
import com.jiubang.p2p.utils.Point;
import com.jiubang.p2p.utils.ScreenUtils;
...
public class Drawl extends View {
	...
	public Drawl(Context context, List<Point> list, String passWord, GestureCallBack callBack) {
		...
		bitmap = Bitmap.createBitmap(display[0], display[1], Bitmap.Config.ARGB_8888); // 设置位图的宽高
		canvas = new Canvas();
		canvas.setBitmap(bitmap);

		paint.setStyle(Style.STROKE);// 设置非填充
		paint.setStrokeWidth(10);// 笔宽5像素
		paint.setColor(context.getResources().getColor(R.color.app_blue));// 设置颜色
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_UP:// 当手指抬起的时候
			...
			callBack.gestureCode(passWordSb.toString());
			// 重置passWordSb
	...
	public interface GestureCallBack {
		 ...
		 * 传出用户绘制的密码
		 */
		public abstract void gestureCode(String code);
	}
